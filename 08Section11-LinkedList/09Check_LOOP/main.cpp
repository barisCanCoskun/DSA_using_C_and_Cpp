...
bool is_LOOP(Node *ptr){
    ...
    if(ptr->next)       // for list of 1 element: (ptr == nullptr == q) in while loop
    {
        do{
            ptr = ptr->next;
            q = q->next;
        ...
        return ptr == q ? true : false;
    }
    return false;
}

Node *create_LOOP(int a[], int n, int pos){
    Node *temp{}, *last{}, *loop_position{};
    Node *firstNode = new Node;
    ++number_nodes;
    firstNode->data = a[0];
    firstNode->next = nullptr;
    ...
    last->next = loop_position;     // make loop
    if(pos == 1)
        last->next = firstNode;
...
int main()
{
    ...
    int position{};
    std::cout << "Enter position to loop:";
    std::cin >> position;
    Node *second = create_LOOP(A, sizeof(A)/sizeof(A[0]), position);
    if(is_LOOP(second))
        std::cout << "This list is loop." << std::endl;
    else
        std::cout << "This list is NOT loop" << std::endl;

    int B[]{7};

    std::cout << "Enter position to loop:";
    std::cin >> position;
    second = create_LOOP(B, sizeof(B)/sizeof(B[0]), position);
    if(is_LOOP(second))
        std::cout << "This list is loop." << std::endl;
    else
        std::cout << "This list is NOT loop" << std::endl;
